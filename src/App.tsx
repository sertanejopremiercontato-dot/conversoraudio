...
import ImageRotateFlip from "./pages/image/ImageRotateFlip";
...
export default function App() {
        ...
        : activeTab === "imageCompressor"
          ...
          : activeTab === "imageResizer"
            ...
            : activeTab === "imageCropper"
              ? "imageCropper"
              : activeTab === "imageRotateFlip"
                ? "imageRotateFlip"
                : activeTab === "imageWatermark"
                  ? "image_watermark"
                  : activeTab === "imageBackgroundRemover"
                    ? "image_background_removal"
                    : activeTab === "pdfExtractText"
                      ? "pdf_extract_text"
  ...
  // Branding (logo and dimensions) state
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setBranding({
          ...
          logoMobileMaxHeight: typeof data.logoMobileMaxHeight === 'number' ? data.logoMobileMaxHeight : 56,
        });
      }
    }, (err) => {
  ...
  useEffect(() => {
    ...
    } else if (window.location.pathname === "/imagem/girar-espelhar") {
      setActiveTab("imageRotateFlip");
    } else if (window.location.pathname === "/imagem/marca-dagua") {
      setActiveTab("imageWatermark");
    } else if (window.location.pathname === "/imagem/remover-fundo") {
      setActiveTab("imageBackgroundRemover");
    } else if (window.location.pathname === "/pdf/extrair-texto") {
  ...
  useEffect(() => {
    ...
    canonicalElement.setAttribute("href", seoConfig.canonical || "https://multiconverte.com.br");
  ...
  const handleNavigatePath = (path: string) => {
    ...
    } else if (path === "/imagem/girar-espelhar") {
      handleNavigate("imageRotateFlip");
    } else if (path === "/imagem/marca-dagua") {
      handleNavigate("imageWatermark");
    } else if (path === "/imagem/remover-fundo") {
    ...
    } else if (path === "/pdf/extrair-texto") {
      ...
      <header data-ads-exclude="true" className="bg-bg-sec border-b border-border-main sticky top-0 z-50 px-4 py-4 md:px-8 shadow-md backdrop-blur-md">
        <div className="max-w-[1220px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          >
            <style>{`
              .header-brand-logo {
                width: ${branding.logoMobileWidth || 180}px;
                max-height: ${branding.logoMobileMaxHeight || 56}px;
                height: auto;
                object-fit: contain;
                max-width: 100%;
                display: block;
              ...
              @media (min-width: 640px) {
                .header-brand-logo {
                  width: ${branding.logoDesktopWidth || 240}px;
                  max-height: ${branding.logoDesktopMaxHeight || 72}px;
                }
              }
            `}</style>
            <img 
            ...
            <AnimatePresence mode="wait">
              {activeTab === "inicio" && (
                ...
                >
                  ...
                  {/* Section: Ferramentas de Documentos */}
                  ...
                  <div className="space-y-6 max-w-6xl mx-auto pt-6 border-t border-border-main/50" id="image-tools-section">
                    ...
                    {/* Five Image Cards Grid */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" id="image-tools-grid">
              ...
              {activeTab === "imageWatermark" && (
                ...
                >
                  <ImageWatermark onNavigate={handleNavigatePath} />
                </motion.div>
              )}

              {activeTab === "imageBackgroundRemover" && (
                <motion.div
                  key="image-background-remover-view"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  transition={{ duration: 0.2 }}
                  className="bg-card-main rounded-[24px] border border-border-main shadow-lg p-6 md:p-10 text-text-main"
                >
                  <ImageBackgroundRemover onNavigate={handleNavigatePath} />
                </motion.div>
              )}
              ...
              {activeTab === "pdf" && (
                <motion.div
                  ...
                  className="bg-card-main rounded-[24px] border border-border-main shadow-lg p-6 md:p-10 text-text-main"
                >
                  <PdfTools activeTool={activePdfTool as any} setActiveTool={setActivePdfTool as any} onNavigate={handleNavigatePath} />
                </motion.div>
              )}

              {activeTab === "pdfExtractText" && (
                <motion.div
                  key="pdf-extract-text-view"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  transition={{ duration: 0.2 }}
                  className="bg-card-main rounded-[24px] border border-border-main shadow-lg p-6 md:p-10 text-text-main"
                >
                  <PdfExtractText onNavigate={handleNavigatePath} />
                </motion.div>
              )}

              {activeTab === "excelToPdf" && (
                <motion.div
                  key="excel-to-pdf-view"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  transition={{ duration: 0.2 }}
                  className="bg-card-main rounded-[24px] border border-border-main shadow-lg p-6 md:p-10 text-text-main"
                >
                  <ExcelToPdf onNavigate={handleNavigatePath} />
                </motion.div>
              )}
              ...
              {activeTab === "documentHub" && (
                <motion.div
                  ...
                  className="bg-card-main rounded-[24px] border border-border-main shadow-lg p-6 md:p-10 text-text-main"
                >
                  <DocumentHub onNavigate={handleNavigatePath} />
                </motion.div>
              )}
              ...
              {activeTab === "imageRotateFlip" && (
                <motion.div
                  ...
                  className="bg-card-main rounded-[24px] border border-border-main shadow-lg p-6 md:p-10 text-text-main"
                >
                  <ImageRotateFlip onNavigate={handleNavigatePath} />
                </motion.div>
              )}
            </AnimatePresence>
