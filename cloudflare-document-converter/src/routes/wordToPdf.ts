...
export async function handleWordToPdf(req: Request, res: Response): Promise<void> {
  const requestId = req.headers['x-request-id'] as string || `word-${Date.now()}`;
  console.log(`[WordToPdf] Nova requisição de conversão [ID: ${requestId}]`);
  ...
  try {
    ...
    const originalFilename = uploadedFile.originalFilename || 'documento.docx';
    ...
    // 3. Validate file
    const validation = FileValidationService.validate(fileBuffer, originalFilename, 'word');
    if (!validation.isValid) {
      res.status(400).json({ error: validation.error || 'Arquivo Word inválido.' });
      return;
    }

    // 4. Create isolated temporary context
    tempContext = TemporaryFilesService.createTempContext(validation.extension || '.docx');
    ...
    if (!result.success || !result.pdfBuffer) {
      res.status(result.statusCode || 500).json({
        error: result.error || 'Falha ao converter documento Word para PDF.',
      });
      return;
    }
    ...
    console.log(`[WordToPdf] Requisição [ID: ${requestId}] finalizada e PDF retornado com sucesso.`);
  } catch (err: any) {
    console.error(`[WordToPdf] Erro na requisição [ID: ${requestId}]:`, err);
    res.status(500).json({
